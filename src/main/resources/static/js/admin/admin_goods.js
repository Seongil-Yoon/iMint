const pageNums = document.querySelector("#pageNums").value;

const url = new URL(window.location.href);
const urlParams = url.searchParams;
let category = urlParams.get("category");
if (urlParams.get("category") == undefined)
    category = "";
let keyword = urlParams.get("keyword");
if (urlParams.get("keyword") == undefined)
    keyword = "";
...
function pagelistReady(pageNums) {
    let html = "";
    if (page == 1) {
        html += `
        <ul class="pagination">
            <li class="page-item">
                <a class="page-link" href="/admin/goods?category=${category}&keyword=${keyword}&page=${page}" aria-label="Previous">
        ...
        <ul class="pagination">
            <li class="page-item">
                <a class="page-link" href="/admin/goods?category=${category}&keyword=${keyword}&page=${page -1}" aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                </a>
            </li>
        `;
    ...
    for (let i = 1; i <= pageNums; i++) {
        if (page == i) {
            ...
            <li class="page-item active" aria-current="page">
                <a class="page-link" href="/admin/goods?category=${category}&keyword=${keyword}&page=${i}">${i}</a>
            </li>
            `
        } else {
            html += `
    ...
    if (page == pageNums) {
        html += `
            <li class="page-item">
                <a class="page-link" href="/admin/goods?category=${category}&keyword=${keyword}&page=${page}" aria-label="Next">
        ...
        html += `
            <li class="page-item">
                <a class="page-link" href="/admin/goods?category=${category}&keyword=${keyword}&page=${page +1}" aria-label="Next">
                    <span aria-hidden="true">&raquo;</span>
                </a>
            </li>
        </ul>
    ...
    $(".goodsPageNums").append(html);
}

function main() {
    pagelistReady(pageNums);
}
main();